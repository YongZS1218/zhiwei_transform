document.addEventListener('DOMContentLoaded', function() {
  ...
  outerSegments.forEach((seg, index) => {
    ...
    // 將初始角度偏移半個格子的角度
    let angle = index * anglePerSegment + anglePerSegment / 2 - 4;
    // 移除反向旋轉
    seg.style.transform = `rotate(${angle}deg) translate(170%)`;
  });
  ...
  middleSegments.forEach((seg, index) => {
    ...
    if (text.length === 1) {
      translateValue = 160; // 單字平移較多
    } else if (text.length === 2) {
      translateValue = 160; // 雙字平移較少
    } else if (text.length === 3) {
      translateValue = 160; // “𩒺”占2個碼元
    }
  ...
  innerSegments.forEach((seg, index) => {
    ...
    if (text.length === 1) {
      translateValue = 125; // 單字平移較多
    } else if (text.length === 2) {
      translateValue = 125; // 雙字平移較少
